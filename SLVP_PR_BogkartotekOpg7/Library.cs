...
namespace SLVP_PR_BogkartotekOpg7
{
    internal class Library
    {
        ...
        /// of Publishers if either is not already there. Finally adds the Book to the Authors and Publishers List of Books.
        /// </summary>
        ...
        public void AddBook(Book book)
        {
            ...
            // Lambda expressions. Finds the Author in the list of Authors whose name matches the Author name of the Book to be added.
            // That Book is then added to that Authors List of Books. Same for Publisher.
            Authors.Find(Auth => Auth.Name == book.Auth.Name).Books.Add(book);
            Publishers.Find(Pub => Pub.Name == book.Pub.Name).Books.Add(book);
        }
        ...
        /// <summary>
        /// Removes the specified Book by Guid from the List of books. If the Book is found then it is also removed from the correct Book Lists in the List
        ...
        public bool RemoveBook(Guid ID)
        {
            ...
            // Removing the book if it exists. If it does then we remove it from the Book Lists in Authors and Publishers too and return true.
            if (Books.Remove(toRemove))
            {
                // Uses the Find method of both Lists with a Lambda expression to find the Author and Publisher with the same name
                // as those of the Book removed. Then removes that Book from both Lists as well.
                Authors.Find(Auth => Auth.Name == toRemove.Auth.Name).Books.Remove(toRemove);
                Publishers.Find(Pub => Pub.Name == toRemove.Pub.Name).Books.Remove(toRemove);
                return true;
            }
            return false;
        }
