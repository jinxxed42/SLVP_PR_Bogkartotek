...
namespace SLVP_PR_BogkartotekOpg7
{
    public partial class Form1 : Form
    {
        ...
        /// <summary>
        /// Event listener for clicks on the Books by author button. Informs the user with a MessageBox if the specified Author wasn't found
        /// or if there are no Books for that Author. Then updates the DataGridView with the data found.
        /// Unlike the Find by author button and it's related FindBooksByAuthor method in the Library class this will find if an 
        /// Author exists, but has no Books.
        /// </summary>
        /// <param name="sender"></param>
        /// <param name="e"></param>
        private void btnBooksByAuthor_Click(object sender, EventArgs e)
        {
            if (tbBooksByAuthor.Text == "")
            {
                MessageBox.Show("Please provide some input and try again.");
            }
            else
            {
                // Getting a List of all Authors and then using the Find method of the List with a Lambda expression
                // to look for an Author name that matches the input.
                List<Author> authList = lib.GetAllAuthors();
                Author tAuth = authList.Find(a => a.Name == tbBooksByAuthor.Text);

                BindingSource bs = new BindingSource();

                // If the result of the Find is null then no matching Author was found.
                if (tAuth is null)
                {
                    bs.DataSource = new { BookID = "", BookTitle = "", AuthorID = "", AuthorName = "", PublisherID = "", PublisherName = "" };
                    MessageBox.Show("The specified author was not found.");
                }
                // Author was found, but no Books were found for that Author.
                else if (tAuth.Books.Count == 0)
                {
                    bs.DataSource = new { BookID = "", BookTitle = "", AuthorID = tAuth.ID, AuthorName = tAuth.Name, PublisherID = "", PublisherName = "" };
                    MessageBox.Show("The specified author exists, but has no books in the database!");
                }
                else
                {
                    // Using a query to setup the DataSource.
                    var bind = from a in tAuth.Books
                               select new
                               {
                                   BookID = a.ID,
                                   BookTitle = a.Title,
        ...
        /// <summary>
        /// Event listener for clicks on the Books by publisher button. Informs the user if the specified Publisher wasn't found
        /// or if there are no Books for that Publisher. Then updates the DataGridView with the data found.
        /// Unlike the Find by publisher button and it's related FindBooksByPublisher method in the Library class this
        /// will find if a Publisher exists, but has no Books.
        /// </summary>
        /// <param name="sender"></param>
        /// <param name="e"></param>
        private void btnBooksByPublisher_Click(object sender, EventArgs e)
        {
            if (tbBooksByPublisher.Text == "")
            {
                MessageBox.Show("Please provide some input and try again.");
            }
            else
            {
                // Getting a List of all Publishers and then using the Find method of the List with a Lambda expression
                // to look for a Publisher name that matches the input.
                List<Publisher> pubList = lib.GetAllPublishers();
                Publisher tPub = pubList.Find(p => p.Name == tbBooksByPublisher.Text);

                BindingSource bs = new BindingSource();

                // If the result of the Find is null then no matching Publisher was found.
                if (tPub is null)
                {
                    bs.DataSource = new { BookID = "", BookTitle = "", AuthorID = "", AuthorName = "", PublisherID = "", PublisherName = "" };
                    MessageBox.Show("The specified publisher was not found.");
                }
                // Publisher was found, but no Books were found for that Publisher.
                else if (tPub.Books.Count == 0)
                {
                    bs.DataSource = new { BookID = "", BookTitle = "", AuthorName = "", AuthorID = "", PublisherName = tPub.Name, PublisherID = tPub.ID };
                    MessageBox.Show("The specified publisher exists, but has no books in the database!");
                }
                else
                {
                    // Using a query to setup the DataSource.
                    var bind = from p in tPub.Books
                               select new
                               {
                                   BookID = p.ID,
                                   BookTitle = p.Title,
                                   AuthorID = p.Auth.ID,
                                   AuthorName = p.Auth.Name,
                                   PublisherID = p.Pub.ID,
                                   PublisherName = p.Pub.Name,
                               };
